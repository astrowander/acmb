...
auto dataset = Registrator::Registrate(IBitmap::Create(GetPathToTestFile("PPM/IMG_4030.ppm")));
...
auto dataset = Registrator::Registrate(IBitmap::Create(GetPathToTestFile("PPM/IMG_4314.ppm")), 50, 5, 25);
auto& stars = dataset->stars;
...
EXPECT_EQ(21, dataset->valuableStarCount);
END_TEST

BEGIN_TEST(Registrator, TestVertical)
auto dataset = Registrator::Registrate(IBitmap::Create(GetPathToTestFile("PPM/vertical.ppm")), 40, 5, 25);
auto& stars = dataset->stars;
