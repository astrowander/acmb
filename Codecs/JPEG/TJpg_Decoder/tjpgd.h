...
struct JDEC {
	...
	std::function<size_t( JDEC*, uint8_t*, size_t )> infunc;	/* Pointer to jpeg stream input function */
	void* device;				/* Pointer to I/O device identifiler for the session */
	uint8_t swap;       /* Added by Bodmer to control byte swapping */
};

/* TJpgDec API functions */
JRESULT jd_prepare (JDEC* jd, std::function<size_t(JDEC*,uint8_t*,size_t)> infunc, void* pool, size_t sz_pool, void* dev);
JRESULT jd_decomp (JDEC* jd, std::function<int(JDEC*,void*,JRECT*)> outfunc, uint8_t scale);
